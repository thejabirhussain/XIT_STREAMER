...
export function ConnectionsPage() {
  ...
  const platformAuthEndpoint: Record<string, string> = {
    youtube: '/auth/youtube/url',
    facebook: '/auth/meta/url',
    instagram: '/auth/instagram/url',
  };

  const connectPlatform = useCallback(async (platformKey: string) => {
    try {
      const endpoint = platformAuthEndpoint[platformKey];
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            <p style={{ fontSize: '14px', color: 'var(--color-text-muted)', margin: 0 }}>
              {platformKey === 'instagram'
                ? 'Connect your Instagram Business account. Requires a Facebook Page linked to an Instagram Business Account.'
                : `Connect your ${cfg.name} account to stream and aggregate chat.`
              }
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
              {platformAuthEndpoint[platformKey] ? (
                <Button
                  variant="primary"
                  size="sm"
                  fullWidth
